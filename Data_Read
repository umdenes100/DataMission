/*
 * The Following code is an example of how students will read the message from the copper plates. The Program 
 * uses the digitalRead command to determine whether the copper plate is displaying a 1 or 0. Next, the bitWrite
 * command stores the collection of 1s and 0s into a variable message. Finally, the message variable, which is 
 * currently written in binary is stored in an array as a char, which prints out the ASCII value of the message.
 */
int inPin = 7;    // input wire is connected to digital pin 7
int delayTime = 125; //delay time must be the same as transmitter(1/8 of a second)
int val = 2;      // variable to store the value read by digitalRead
char a[0]; //char array to convert message from binary to ASCII

int lastvals[8] = {2,2,2,2,2,2,2,2};     //Store the contents of the last 8 values
...
void loop() {
  ...
  if(lastvals[7] == 0 && lastvals[6] == 0 && lastvals[5] == 0 && lastvals[4] == 0 && lastvals[3] == 0 && lastvals[2] == 0 && lastvals[1] == 0 && lastvals[0] == 0 && val == 1){
    ...
    Serial.print(a);     //print the ASCII value of the message
  }
